...
struct RepoMembersView: View {
    ...
    var body: some View {
            ...
            ZStack{
                if userList.count == 0 && !isLoading {
                    VStack{
                        ...
                        Text("暂无查询到的仓库成员").padding(.top,30)
                    }
                }
        ...
        .navigationBarTitle(Text("仓库成员列表"), displayMode: .inline)
        .onAppear(){
            self.page = 1
            self.getUserList( page: self.page)
        }
    ...
    func getUserList(page: Int){
        ...
        let url = "repos/" + repoPath + "/collaborators?page=" + String(page);
