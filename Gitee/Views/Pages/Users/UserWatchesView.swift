...
struct UserWatchesView: View {
    ...
    var body: some View {
            ...
            ZStack{
                if userList.count == 0 && !isLoading {
                    VStack{
                        ...
                        Text("暂无用户Watch这个仓库").padding(.top,30)
                    }
                }
        ...
        .navigationBarTitle(Text("Watch用户列表"), displayMode: .inline)
        .onAppear(){
            self.page = 1
            self.getUserList( page: self.page)
        }
    ...
    func getUserList(page: Int){
        ...
        let url = "repos/" + repoPath + "/subscribers?page=" + String(page);
