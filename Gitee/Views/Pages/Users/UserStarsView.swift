...
struct UserStarsView: View {
    ...
    var body: some View {
            ...
            ZStack{
                if userList.count == 0 && !isLoading {
                    VStack{
                        ...
                        Text("暂无用户Star这个仓库").padding(.top,30)
                    }
                }
        ...
        .navigationBarTitle(Text("Star用户列表"), displayMode: .inline)
        .onAppear(){
            self.page = 1
            self.getUserList( page: self.page)
        }
    ...
    func getUserList(page: Int){
        ...
        let url = "repos/" + repoPath + "/stargazers?page=" + String(page);
